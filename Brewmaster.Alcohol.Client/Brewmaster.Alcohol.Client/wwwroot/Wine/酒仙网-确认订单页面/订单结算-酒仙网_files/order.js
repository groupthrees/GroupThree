...
$('.pubClose,.cancel').bind('click',function(){
     ...
     if(type == 'gift'){
        ...
        if($.trim($("#isInvoice").val()) == '0'){
           ...
           || type=='unableProduct' || type=='stopTypeWeather'){//无货赠品和运营停发和支付密码
         isSubmit= true;
...
function loadGoods(isLoadCoupon){
	...
	$.ajax({
		...
		type:'get',
        cache:false,
		data:{regionId:regionId,stockoutGifts:cartStockoutGifts},
		success:function(data){
			if(data.code == '0'){//加载成功
...
function loadCoupons(selectedId,type){
    ...
    $.ajax({
        ...
        dataType:'html',
        cache:false,
		data:{regionId:regionId,selectedIds:checkCouponIds,selectedId:selectedId,isCheck:type,useAllowance:useAllowance},
        success:function(data){
            $("#couponListDiv").empty();
